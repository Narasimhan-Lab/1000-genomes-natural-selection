...
# population info here # e.g. target = "C", source1 = "A", source2 = "B" for the model C = A + B
target = "EN"
source1 = "M"
source2 = "AN"
...
pop_proportions = [0.237, 0.763]
assert (np.sum(pop_proportions) == 1)

multiplier = 1.96

# qpAdm standard error (jackknife)
prop_se = 0.003
...
# bounds from sampling
'''
first_prop = [0.161, 0.839]
second_prop = [0.165, 0.835]
assert (np.sum(first_prop) == 1)
assert (np.sum(second_prop) == 1)
'''
...
ref_path = mat_prefix + "refAlleles.hdf5"
alt_path = mat_prefix + "altAlleles.hdf5"

# indices of samples in hdf5 file for each population
target_npy = data_prefix + target + "_indices.npy"
s1_npy = data_prefix + source1 + "_indices.npy"
s2_npy = data_prefix + source2 + "_indices.npy"
...
# highest resutls after perturbation
...
# to apply negative log to an array
